...
// Sheets reuse the shared renderer so the standalone package picker and these
// inventories cannot drift apart visually.
type sheetColumn = share.SheetColumn

var (
	sheetBorder  = share.SheetBorder
	sheetHeading = share.SheetHeading
)

func cell(value string, width int) string { return share.SheetCell(value, width) }
func grid(columns []sheetColumn, rows [][]string, selected int) []string {
	return share.SheetGrid(columns, rows, selected)
}

func sheetTitle(title string, width int, focused bool) string {
	return share.SheetTitle(title, width, focused)
}

func sheetView(title string, columns []sheetColumn, rows [][]string, cursor, width, height int, focused bool, footer string) string {
	return share.SheetView(title, columns, rows, cursor, width, height, focused, footer)
}
...
func propertySheet(title string, fields [][2]string, width, height, offset int, focused, tabbable bool) string {
	...
	columns := []sheetColumn{{Name: "항목", Width: keyWidth}, {Name: "값", Width: valueWidth}}
	var rows [][]string
...
func (m model) inventoryPanes(width, height int) string {
	...
	columns := []sheetColumn{{Name: nameHeader, Width: inner - groupWidth - stateWidth}, {Name: "GROUP", Width: groupWidth}, {Name: "STATE", Width: stateWidth}}
